...
const Header = () => {
  ...
  return (
    <>
      <nav className={`absolute md:fixed top-0 w-full z-50 transition-all duration-300 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl ${scrolled ? 'shadow-[0px_20px_40px_rgba(87,73,194,0.06)]' : ''}`}>
        <div className="flex justify-between items-center px-4 md:px-8 py-4 max-w-7xl mx-auto font-headline antialiased">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <Link to="/get-started" className={`font-semibold transition-colors duration-300 ${location.pathname.startsWith('/get-started') ? 'text-primary' : 'text-on-surface-variant dark:text-slate-400 hover:text-primary'}`}>{t('header.demo')}</Link>
          </div>
